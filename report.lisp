...
(defmethod eval-in-context :around ((report interactive) (result result))
  (restart-case
      (call-next-method)
    (retry ()
      :report #.(lformat "Retry testing ~a" (format-result result :oneline))
      (eval-in-context report result))
    (abort ()
      :report #.(lformat "Continue, failing ~a" (format-result result :oneline))
      (setf (status result) :failed))
    (continue ()
      :report #.(lformat "Continue, skipping ~a" (format-result result :oneline))
      (setf (status result) :skipped))
    (pass ()
      :report #.(lformat "Continue, passing ~a" (format-result result :oneline))
